...
    // Evento para obtener la referencia del API
    datesSet: (info) => {
...
        // Aquí this.datosCita.fecha_cita se asigna con un string válido
...
        console.log('[CITA FORM] Modo Seleccionar Hora activado. Fecha:', this.fechaPreseleccionada);
        
...
  }
  
  // Manejador para evitar que el usuario cambie la fecha en modo selección de hora
  handleDayNavigationClick(arg: any): void {
      if (!this.modoSeleccionHora) return;

      const dateString = arg.dateStr.split('T')[0];
      
      // Corregido: Si la fecha preseleccionada es null, salimos o usamos un fallback
      if (!this.fechaPreseleccionada) return;

      // Si la fecha clickeada es diferente a la preseleccionada, se bloquea la navegación
...
          });
          // Forzamos al calendario a volver a la fecha original
          if (this.calendarApi) {
                // Corrección: Usamos '!' ya que hemos comprobado que no es null
            this.calendarApi.gotoDate(this.fechaPreseleccionada!);
          }
      } else {
         // Si es la fecha correcta, la dejamos (aunque solo se permite la vista de día)
...
    if (this.paso === 2) {
      // Si estamos en el paso 2 (Datos de Cita), retrocedemos al paso 1 (Validar Cliente)
      this.volverPaso1();
      console.log('[CITA FORM] Retrocediendo de Paso 2 a Paso 1 (Validar Cliente).');
    } else if (this.modoSeleccionHora) {
      // Si estamos en modo de selección de hora (lo cual implica que venimos de solicitarcita), volvemos allí
      console.log('[CITA FORM] Volviendo a Solicitar Cita para cambiar la fecha.');
      this.router.navigate(['/solicitar-cita']);
    } else {
      // Si estamos en el paso 1 (Validar Cliente) en modo normal, navegamos al dashboard
...
        this.paso = 2; // Avanzamos al paso 2 (Registro de Cita/Mascota)
        
        // Si no estamos en modo selección de hora (o sea, es un registro completo), cargamos el calendario
        if (!this.modoSeleccionHora) {
             this.cargarCitasParaCalendario(); // Modo normal, carga el calendario completo.
        }
      },
      error: (err: Error) => {
        this.cargando = false;
...
  // Se mantiene la lógica de clases de día, pero en modo selección de hora, es menos relevante
...
    }
    
    // En modo selección de hora, si no es la fecha, la oscurecemos
    // Corrección: Comprobamos this.fechaPreseleccionada para evitar errores
    if (this.modoSeleccionHora && this.fechaPreseleccionada && fechaDia !== this.fechaPreseleccionada) {
...
          
          // Solo hacemos visible el calendario si no estamos en modo de selección de hora (o si ya se configuró)
          if (!this.modoSeleccionHora) {
            this.calendarVisible = true;
          } else {
            // Si estamos en modo de selección de hora, ya se configuró en ngOnInit, solo actualizamos eventos.
          }
...
        // Configuramos el calendario en vista de día después de cargar las citas
...
    // Filtramos solo si estamos en modo selección de hora y tenemos una fecha fija
    // Corrección: Comprobamos this.fechaPreseleccionada para evitar errores
...
        });
        console.log(`[CITA FORM] Mostrando ${citasFiltradas.length} citas para el día fijo: ${this.fechaPreseleccionada}`);
    }
...
    };
    
    // Corrección: Comprobamos initialDate para evitar el error de tipo en gotoDate
...
    // Si estamos en modo de selección de hora (viene de solicitarcita)
    if (this.modoSeleccionHora) {
...
    }
    
    // Lógica normal (viene del paso 1 de este mismo componente)
...
    // Si no estamos en modo selección de hora, cambiamos la vista a día
    if (this.calendarApi) {
...
    // Si estamos en modo de selección de hora, aseguramos que la fecha sea la preseleccionada
    // Corrección: Comprobamos this.fechaPreseleccionada para evitar errores
    if (this.modoSeleccionHora && this.fechaPreseleccionada && fechaISO !== this.fechaPreseleccionada) {
         Swal.fire({
            icon: 'error',
...
    // Corrección: Usamos '!' ya que la cita contiene estos campos como string
...
    // Corrección: Comprobamos this.datosCita.fecha_cita antes de usarlo
...
        this.cargando = false;
        
        const message = err.message.includes('Error: ') ? err.message.replace('Error: ', '') : err.message;
